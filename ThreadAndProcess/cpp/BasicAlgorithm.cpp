...
void BasicAlgorithm::QuickSort(int* h, int left, int right)
{
	//快速排序，随机选取哨兵放前面 
...
void BasicAlgorithm::BubbleSort(int* h, size_t len)
{
	if (h == NULL) return;
	if (len <= 1) return;
	//i是次数，j是具体下标 
...
void BasicAlgorithm::SelectionSort(int* h, size_t len)
{
	if (h == NULL) return;
	if (len <= 1) return;
...
void BasicAlgorithm::InsertSort(int* h, size_t len)
{
	if (h == NULL) 
		return;
	if (len <= 1) 
...
void BasicAlgorithm::MergeArray(int* arr, size_t left, size_t mid, size_t right, int* temp)
{
	if (arr == NULL) 
		return;
...
void BasicAlgorithm::MMergeSort(int* arr, size_t left, size_t right, int* temp)
{
	if (left < right)
	{
...
void BasicAlgorithm::MergeSort(int* h, size_t len)
{
	if (h == NULL) return;
	if (len <= 1) return;
...
void BasicAlgorithm::ShellSort(int* h, size_t len)
{
	if (h == NULL) return;
	if (len <= 1) return;
...
void BasicAlgorithm::AdjustHeap(int* h, int node, int len)
{
	int index = node;
...
void BasicAlgorithm::MakeHeap(int* h, int len)
{
...
void BasicAlgorithm::HeapSort(int* h, int len)
{
	MakeHeap(h, len);
...
int BasicAlgorithm::GetMaxDight(int* h, int len)
{
	if (h == NULL) return 0;
	if (len < 1) return 0;
...
int BasicAlgorithm::GetReminder(int value, int digit)
{
	int div = 1;
...
void BasicAlgorithm::RadixSort_LSD(int* h, int len)
{
	if (h == NULL) return;
	if (len <= 1) return;
...
void BasicAlgorithm::RadixSort_LSD_Reverse(int* h, int len)
{
	if (h == NULL) return;
	if (len <= 1) return;
...
void BasicAlgorithm::RRadixSort_MSD(int* h, int begin, int end, int digit)
{
	if (h == NULL) return;
	if (begin >= end) return;
	if (digit < 1) return;
...
void BasicAlgorithm::RadixSort_MSD(int* h, int len)
{
	if (h == NULL) return;
	if (len <= 1) return;
...
void BasicAlgorithm::RRadixSort_MSD_Reverse(int* h, int begin, int end, int digit)
{
	if (h == NULL) return;
	if (begin >= end) return;
	if (digit < 1) return;
...
void BasicAlgorithm::RadixSort_MSD_Reverse(int* h, int len)
{
	if (h == NULL) return;
	if (len <= 1) return;
...
void BasicAlgorithm::CSerialPort()
{
